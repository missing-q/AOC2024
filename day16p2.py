...
while open_list:
    ...
    # forward
    forward = direction.backTrack(newcoord)
    if distances[forward, direction] == num - 1:
        allpaths.add(forward)
        open_list.add((forward, direction))

    # cw
    cw = direction.turnCW() 
    if distances[newcoord, cw] == num - 1000:
        open_list.add((newcoord, cw))

    # ccw
    ccw = direction.turnCCW() 
    if distances[newcoord, ccw] == num-  1000:
        open_list.add((newcoord, ccw))
